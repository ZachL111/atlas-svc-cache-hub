...
`atlas-svc-cache-hub` is centered on backend services work in Kotlin. The technical target is: Design a Kotlin verification harness for cache systems, covering policy evaluation, deny and allow fixtures, and failure-oriented tests.
...
The score is deterministic. The current threshold is 158. Risk has a penalty of 7, latency has a penalty of 2, and weight contributes a bonus of 3.
